...
//%debug
%unicode
%type org.embulk.spi.time.RubyTimeFormatToken
%{
  private StringBuilder buffer = new StringBuilder();

  private RubyTimeFormatToken emitRawStringToken() {
    final String string = buffer.toString();
    buffer.setLength(0);
    return new RubyTimeFormatStringToken(string);
  }

  private RubyTimeFormatToken emitFormatDirective(final char specifier) {
    RubyTimeFormatToken token;
    if (specifier == 'z') {
      int colons = yylength() - 1; // can only be colons except the 'z'
      return new RubyTimeFormatTimeZoneOffsetToken(colons);
    } else if ((token = RubyTimeFormatToken.getFormatDirectiveToken(specifier)) != null) {
      return token;
    } else {
      return new RubyTimeFormatSpecialToken(specifier);
    }
  }
%}
...
<YYINITIAL> {
  {LiteralPercent}                  { return new RubyTimeFormatStringToken("%"); }
